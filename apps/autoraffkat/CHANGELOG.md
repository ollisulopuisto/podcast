...
### Fixed
- **Short vertical close-ups were left centred, cutting faces in half** (`reframe.py`): a shot needed three measured frames of its own, so under ~3 s it got no framing — 252 of 594 clips on the real episode, 30–100 % of the face outside the crop. The position comes from the camera's steady track; the shot's own frames no longer gate it. Reaction shots had the same problem. A shot whose own median face box would still cross the edge (a whole-shot lean) moves just enough to keep it.
...
### Changed
- **Renders on the GPU on macOS** (`render_av.py`): AVFoundation decodes, transforms and encodes the picture like Final Cut — a fifth of the CPU of the ffmpeg path — in two halves in parallel for the machine's two hardware encoders (~16× real time on an M1 Max). ffmpeg remains the fallback and the Windows path; the terminal log names the backend.
