...
autotui
```
*(Or `uv run --package automixer python -m automixer.app` if running from source)*
...
- `src/automixer/domain/`: Session-level DSP (Processors, Buses, Tracks) and the routing.
...
Everything that turns speech samples into other speech samples lives in
[`packages/speechmix`](../../packages/speechmix) instead, shared with the
sibling apps in this workspace. The measured findings behind it are in
[`FINDINGS.md`](../../packages/speechmix/FINDINGS.md).
