...
uv run --package automixer pytest apps/automixer/tests
```
The tests import `automixer`, the installed package -- not a path relative to
the checkout. Importing through the checkout path is what hid a broken package
layout for as long as it did.
