...
from .detect import AudioCache, speech_intervals
...
def preview(session_path: str, settings: Settings, extra_dir: str = "") -> dict:
    ...
    for track in session.tracks:
        result = speech_intervals(session, track, settings, cache, extra_dir)
...
def run(
    ...
    for index, track in enumerate(session.tracks):
        ...
        progress.log(
            f"  {track.name}: {result.words_seen} sanaa{quiet_note} → "
