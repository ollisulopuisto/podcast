...
Ajetaan `build_app.py`:n jälkeen: se tekee `dist/Podcast Magic.app`, tämä
...
def make_dmg(app_path: Path | None = None, output: Path | None = None) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="podcast-magic-dmg-") as staging_dir:
        ...
        for icon_name in ("podcast-magic.icns", "icon.icns"):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Pakkaa dist/Podcast Magic.app jaettavaksi levykuvaksi."
    )
    parser.add_argument(
        "--app",
        type=Path,
        default=None,
        help="sovelluspaketti (oletus dist/Podcast Magic.app)",
    )
    parser.add_argument(
        "-o",
        "--output",
        type=Path,
        default=None,
        help="kohdetiedosto (oletus dist/Podcast Magic.dmg)",
    )
