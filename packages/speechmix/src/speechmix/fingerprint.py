...
out unnoticed: ``fingerprint`` refuses a settings object that is missing any
field it names, and the test suite fails on a settings mapping that has grown
one the list does not.  Where the settings themselves live is the host's
business -- they are an app concern -- but *which* of them change the result is
the chain's.
...
def fingerprint(settings, extra=None):
    ...
    Args:
        settings: Any mapping of setting name to value, or anything with an
            ``as_dict()``.  The host owns the settings object; this only cares
            that every field named below is present in it.
    ...
    Returns:
        A hex digest.
    """
    values = settings.as_dict() if hasattr(settings, "as_dict") else dict(settings)
