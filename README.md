...
| If you want to... | Use | What it works with |
|---|---|---|
| **Cut multicam video** automatically to whoever is talking | [autoraffkat](#autoraffkat) | Final Cut Pro (`.fcpxml` / `.fcpxmld`) |
| **Transcribe & silence bleed** locally on your Mac | [podcast-magic](#podcast-magic) | Hindenburg (`.nhsx`) |
| **Mix & master** multi-track speech and music to -16 LUFS | [automixer](#automixer) | WAV / AIFF audio files |
| **Transcribe & silence bleed** using a cloud GPU | [colab-transcribe](#colab-transcribe) | Hindenburg (`.nhsx`) + Google Colab |
| **Render or inspect** a session without Hindenburg | [nhsx-render](#nhsx-render) | Hindenburg (`.nhsx`) → WAV |
| **Quick Look preview** sessions in macOS Finder | [NHSX Viewer](#nhsx-viewer--quick-look) | Hindenburg (`.nhsx`) |
...
* **How to run:**
  ```bash
  uv run autoraffkat                      # picks newest export in current folder
  uv run autoraffkat "episode.fcpxmld"    # open a specific export
  uv run autoraffkat --pick               # open Finder file dialog
  ```
  Opens a browser UI at `http://127.0.0.1:8731/` with a full-episode preview, instant slider adjustments for turn thresholds and overlap rules, and one-key export (`⌘E`).
...
* **How to run:**
  ```bash
  uv run podcast-magic ~/Podcast/episode8/   # browser interface at localhost:8741
  uv run podcast-magic --gui                 # native desktop window
  ```
...
* **How to run:**
  ```bash
  uv run autotui                             # interactive terminal dashboard
  uv run automixer                           # CLI mixer
  ```
...
* **How to run:**
  ```bash
  uv run colab-transcribe                    # interactive TUI
...
* **The Solution:** A standalone CLI tool built with pure Python, NumPy, and ffmpeg. Reads `.nhsx` XML and audio files to render a complete mix (respecting track volume, pan, fades, and mutes). The `--plan` and `--json` flags inspect the mix in milliseconds without decoding any audio.
* **How to run:**
  ```bash
...
* **The Solution:** A native Swift application and Quick Look extension. Select any `.nhsx` file in Finder and hit **Spacebar** to see the timeline, track layout, and play back the mix immediately.
